...
void Server::setUser(int fd, std::stringstream& iss) {
    ...
    msg += ":" + _hostname + " 376 " + this->list[fd].getNick() + " :End of MOTD command\r\n";
		...
		// this->sendMessage(fd, ":IRC_SERVER 001 " +  this->list[fd].getNick() +  " :Welcome to the Internet Relay Network " + this->list[fd].getNick() + "!~" + this->list[fd].getUser() + "@" + this->list[fd].getIP() + "\r\n");
